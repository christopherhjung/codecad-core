...
class PlaneSlice(val face: RoutedFace, val plane: Plane, var start: Edge? = null, var end: Edge? = null, var startPosition : PointD? = null, var endPosition: PointD? = null)

class EdgeSlice(val a: PointD? = null, val b: PointD? = null, val plane: Plane?){
...
class Intersection(val edge : Edge, val position: PointD)

val edgeSlices = HashMap<EdgeSlice, PointD>()

fun findIntersections(face: RoutedFace, plane: Plane) : List<Intersection>{
    val result = mutableListOf<Intersection>()
    for(edge in face){
        val start = edge.source.point
        val end = edge.target.point
        ...
        if(event.start){
            ...
            for (other in otherActive.values) {
                ...
                for(intersection in leftIntersections){
                    intersectionEvents.add(Event(intersection, line.direction.dot(intersection.position), 0))
                }
                for(intersection in rightIntersections){
                    intersectionEvents.add(Event(intersection, line.direction.dot(intersection.position), 1))
                }
                ...
                for(intersectionEvent in intersectionEvents){
                    ...
                    if(inside[currentIndex] && inside[ otherIndex ] ){
                        ...
                        if(last[otherIndex]!!.position.squaredDistanceTo(intersectionEvent.intersection.position) < 1e-8){
...
fun applyPlaneSlices(slices: List<PlaneSlice> , assignmentTable : Map<RoutedFace, Owner>) : FaceAssignment{
    ...
    for((face, ops) in map.entries){
        val nodeMap = mutableMapOf<PointD, Corner>()
        ...
        for( slice in ops.slices ){
            if(slice.start?.source?.point === slice.startPosition ){
                if(slice.end?.target?.point === slice.endPosition){
                    continue
                }
            }else if(slice.start?.target?.point === slice.startPosition){
                if(slice.end?.source?.point === slice.endPosition){
                    continue
                }
            }
...
fun addVolumes(base: Volume, tool: Volume) : FacedVolume{
    ...
    return FacedVolume(baseFaces + toolFaces.map {
        when (it) {
            is PolygonFace -> PolygonFace(it.positions.reversed(), it.type, it.plane.flip())
