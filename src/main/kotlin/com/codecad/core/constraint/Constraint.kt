...
class Horizontal(val line: LineSegmentExpr) : Constraint() {
    override fun equationImpl(): Expr {
...
class Vertical(val line: LineSegmentExpr) : Constraint() {
    override fun equationImpl(): Expr {
        //val direction = line.p1 - line.p0
...
class CircleTangent(val circle: SketchConic, val line : LineSegmentExpr) : Constraint() {
    override fun equationImpl(): Expr {
...
fun pointAlongLine(line: LineSegmentExpr, r: Expr): Vec2Expr {
    return line.p0 + line.difference * r
}

fun projectionFactorBetween(line: LineSegmentExpr, point: Vec2Expr): Expr {
...
fun projectOntoLine(line: LineSegmentExpr, point: Vec2Expr): Vec2Expr {
...
class Perpendicular(val line1: LineSegmentExpr, val line2: LineSegmentExpr) : Constraint() {
    override fun equationImpl(): Expr {
        return (line1.direction.normalized().dot(line2.direction.normalized())).pow(2)
    }
}

class Parallel(val line1: LineSegmentExpr, val line2: LineSegmentExpr) : Constraint() {
    override fun equationImpl(): Expr {
        return line1.direction.crossZ(line2.direction).pow(2)
    }
}

class Colinear(val line1: LineSegmentExpr, val line2: LineSegmentExpr) : Constraint() {
    override fun equationImpl(): Expr {
...
class PointOnLine(val point: Vec2Expr, val line: LineSegmentExpr) : Constraint() {
...
class PointOnLineMidpoint(val point: Vec2Expr, val line: LineSegmentExpr) : Constraint() {
    override fun equationImpl(): Expr {
        return (line.midPoint - point).squaredLength()
    }
}

class InternalAngle(val line1: LineSegmentExpr, val line2: LineSegmentExpr, val angle: Expr) : Constraint() {
    override fun equationImpl(): Expr {
